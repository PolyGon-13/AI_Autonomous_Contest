...
float Kp = 0.5f, Ki = 0.25f, Kd = 0.2f;           // PID 상수
...
void analyze_Line() {
  ...
  int targetTotal = leftCount + rightCount;
  lineDetected = (targetTotal >= MIN_DARK_PIXELS);

  // [안전] 분모 0 방지
  if (targetTotal > 0) {
    CENTER_LINE = (leftSum + rightSum) / (float)targetTotal;
  } else {
...
void PID_control() {
  ...
  if (!lineDetected) {                 // 라인인식 불가시 적분항 감쇠
    error = 0.0f;
    integral *= decayRate;
  }
...
void print_Serial() {
  ...
  Serial.print(" darkTotal=");     // 이름 유지(= 목표 픽셀 수)

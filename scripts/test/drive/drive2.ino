...
const int MIN_CONTRAST = 500;           // 최소 콘트라스트(vmax - vmin) 기준
...
int count = 0;                  // 주행용 카운트(0: 장애물 인식X, 라인O, 벽O / 1: 장애물 인식O, 라인O, 벽X)
bool wall_work = false;
bool turn = false;
float currentPos; 

int object_distance = 400;  //장애물 사이 거리
  ...
  switch (state) {
    case AVOID_TURN: {
      ...
      if (straightStart == 0) {
        ...
        if (m3.RangeMilliMeter < object_distance) {
          turn = true;
        }else if (turn && (m3.RangeMilliMeter > object_distance || m3.RangeStatus == 4)) { // 장애물을 지나쳤다고 판단되면 직진 단계로 전환
    ...
    case AVOID_RIGHT: {
      ...
      if (!turnDone) {
        // N초 동안 우회전
        move_right();
        if (millis() - straightStart >= TURN_DURATION_MS) { // N초 우회전 완료
          turnDone = true;
...
void wall(){ 
  ...
  if (lineDetected){  // 라인 인식시 while문 벗어나고 count 1로 지정
    count = 1;       // 벽 인식시 count 1로 지정
  }
}
...
void Drive() {
  ...
  if (state == AVOID_TURN || state == AVOID_RIGHT || state == AVOID_RETURN) { // 회피 중
    avoid();
  }

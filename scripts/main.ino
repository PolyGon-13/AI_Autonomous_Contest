...
const unsigned long EXPOSURE_US = 4000; // 적분(노출) 시간(us)
const int MARGIN = 0;                 // 임계값 보정 여유(흰선 인식시 음수값으로 조정)
static uint32_t last_si_us = 0;         // 고정 적분 주기 유지용 타임스탬프
const int MIN_CONTRAST = 300;           // 최소 콘트라스트(vmax - vmin) 기준
...
const int MIN_DARK_PIXELS = 10;          // '목표' 픽셀 최소 개수 기준(이제 흰선 기준일 때 밝은 픽셀 수)
...
float Kp = 2.0f, Ki = 0.1f, Kd = 0.8f; // PID 상수
...
int wall_distance = 4000; // 벽 인식 거리

int object_backward = 200; // 장애물 최초 인식하기 위한 거리
int object_distance = 500; // 장애물 회피하기 위해 후진하는 거리

long fuck_time;

bool fuck_check = true;

// 장애물 회피
static unsigned long straightStart = 0;
const unsigned long STRAIGHT_DURATION_MS = 0; // 직진 유지할 시간
const unsigned long TURN_DURATION_MS = 4000;    // 우회전 유지할 시간
...
int speed;               
int drive_speed = 1000;					// 직진 속도
int object_speed = 600;         // 장애물 속도
	...
	switch (state) {
		...
		case AVOID_TURN: {
			...
			if (straightStart == 0) {
				...
				if (turn && (m3.RangeMilliMeter > object_distance || m3.RangeStatus == 4)) {
...
void Drive() {
	...
	if (line_wall_mode) { //======================================== 벽 주행 ===================================
		...
		if (turn3) {
			...
			if (!lineDetected) {
				wall_P = ((m1.RangeMilliMeter - 800) * wall_Kp) * Ks;
				wall_steering_device = roundf(constrain(DEG_CENTER - wall_P, DEG_LEFT, DEG_RIGHT));
				dxl.setGoalPosition(ID_steering, wall_steering_device, UNIT_DEGREE);
		...
		if (lineDetected && !wall_work) {
			...
			if (line_wall_mode && !wall_work /* && !turn3 */ 
				...
				if (m3.RangeMilliMeter > wall_distance + 100 || m3.RangeStatus == 4) {
					if (!turn1) {                // ← 최초 진입에서만 타이머 스타트
						fuck_time = millis();
					}
					if (!turn1 || !check) {
						turn1 = true;
					}
					if (turn1 && check && !turn2 && m2.RangeMilliMeter <= 1200) {
						turn2 = true;
						check = false;
					}
				} if (turn1 && !turn2 && !turn3 && millis() - fuck_time  >= 7000 ) {
					check = true;
				} else if (turn1 && !check && turn2 && !turn3 && (m2.RangeMilliMeter > wall_distance || m2.RangeStatus == 4)) {
					turn3 = true;
				}
			}
		}
		...
		if (line_object_mode && m2.RangeMilliMeter < object_backward && state == BASIC) {
			...
			stop();
			avoid();
		} else if (state == AVOID_BACK || state == AVOID_TURN || state == AVOID_RIGHT || state == AVOID_RETURN) { // 회피 중
			speed = object_speed;
			avoid();
		} 
	}
